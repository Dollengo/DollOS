...
helpcommand = ("""[bold] Comandos: [/bold]\n*USE . for any commands.\n\n[italic]
...
def start():
    ...
    print(""" [bold] [italic]
    DDDD   OOO  L     L           OOO  SSSS
    D   D O   O L     L          O   O S
    D   D O   O L     L          O   O  SSS
    D   D O   O L     L          O   O     S
    DDDD   OOO  LLLLL LLLLL       OOO  SSSS 
    [/bold] [/italic]\n""")
...
def handle_command(command):
    ...
    if cmd in command_map:
        ...
        print(f"[bold red]Unknown command: {cmd}[/bold red]")
        print(f"[italic red] - {cmd} - [/italic red]")
...
def desktop():
    ...
    print(f" {desktoparray[12]}  |  {desktoparray[13]}  |  {desktoparray[14]}  |  {desktoparray[5]}")

# Main loop
def main_loop():
    try:
        while True:
            command = input(f"{current_directory}: ").lower()
